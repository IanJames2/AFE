...
export const SchoolDetail = () => {
  ...
  return (
    <section className="school">
      ...
      <div className="school__schoolTypeId">School Type: {school.schoolType.name}</div>
      <div className="school__logo">School Logo: {school.logo}</div>
      <div className="school__major">Field of Study: {school.major}</div>
      <div className="school__tuition">Annual Cost of School: ${school.tuition}</div>
      <div className="school__creator">Inquired by: {school.user?.name}</div>
      <button className="delete_button" onClick={deleteSchoolInvoked}>Delete School</button>
